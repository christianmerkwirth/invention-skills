"""CLI wrapper to install skills for Claude Code."""
...
from install.lib.targets.claude_code import emit, EmittedFile
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Install skills for Claude Code.")
    parser.add_argument(
        "--target",
        type=Path,
        default=Path.home() / ".claude" / "skills",
        help="Target directory (defaults to ~/.claude/skills)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
