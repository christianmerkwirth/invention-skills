...
def test_emit_success(tmp_path):
    ...
    emitted = emit(skill, target_root, reference_files)
    
    # Should emit 1 manifest + 1 command toml + 2 ref files = 4 files
    assert len(emitted) == 4
    
    emitted_dict = {ef.path: ef.content for ef in emitted}
    
    skill_dir = target_root / "test-skill"
    
    # Check manifest
    manifest_path = skill_dir / "gemini-extension.json"
...
def test_emit_escapes_quotes_in_description(tmp_path):
    ...
    toml_path = target_root / "test-skill" / "commands" / "test-skill.toml"
    emitted_dict = {ef.path: ef.content for ef in emitted}
    toml_content = emitted_dict[toml_path].decode("utf-8")
...
def test_creates_extension_per_skill(repo_layout: Path, target_dir: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("install.install_gemini_cli._repo_root", lambda: repo_layout)
    rc = main(["--target", str(target_dir)])
    assert rc == 0
    ext_dir = target_dir / "triz-contradiction"
...
def test_manifest_keys(repo_layout: Path, target_dir: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    manifest_path = target_dir / "triz-contradiction" / "gemini-extension.json"
...
def test_command_toml_has_prompt(repo_layout: Path, target_dir: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("install.install_gemini_cli._repo_root", lambda: repo_layout)
    main(["--target", str(target_dir)])
    toml_path = target_dir / "triz-contradiction" / "commands" / "triz-contradiction.toml"
    content = toml_path.read_text(encoding="utf-8")
    assert 'prompt = """\n# TRIZ Contradiction\n\nLook up `reference/parameters.md`.\n\n"""' in content

def test_reference_bundled_per_extension(repo_layout: Path, target_dir: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("install.install_gemini_cli._repo_root", lambda: repo_layout)
    main(["--target", str(target_dir)])
